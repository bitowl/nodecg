...
module.exports = function (pathOrHtml, resourceType, opts, cb) {
	...
	function inject(err, html) {
		...
		scripts.push('<script src="/bower_components/webcomponentsjs/webcomponents-loader.js"></script>');
		...
		if (resourceType === 'panel' || resourceType === 'dialog') {
			...
			if (!opts.fullbleed) {
				scripts.unshift('<script async src="/bower_components/iframe-resizer/js/iframeResizer.contentWindow.js">' +
					'</script>');
			}
